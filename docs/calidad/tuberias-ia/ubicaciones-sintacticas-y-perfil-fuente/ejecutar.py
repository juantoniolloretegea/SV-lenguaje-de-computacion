...
for c in cases:
 ...
 rows.append('('+','.join([lit(c['id']),lit(c['source']),'P::'+('En' if c['profile']=='en' else 'Es'),lit(c['cause']),span])+')')
...
fn main(){for (id,s,p,cause,span) in [ROWS] {check(id,s,p,cause,span);}}
'''.replace('ROWS',',\n'.join(rows))
