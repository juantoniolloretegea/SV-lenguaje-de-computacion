...
// Cotejo de recepción; no ejecuta el reconocedor.
// Reception identity check; it does not execute the recognizer.
fn main() {
 use std::{fs,path::Path};
 auditbase::selfcheck();
 let args:Vec<String>=std::env::args().collect();
 assert_eq!(args.len(),3,"uso: cotejo DIRECTORIO_CON_COPIAS MANIFIESTO_ENTRADAS");
 let dir=Path::new(&args[1]);
 let entries=fs::read_to_string(&args[2]).unwrap();
 let mut observed=std::collections::BTreeMap::new();
 println!("ruta\tbytes\tblob_git\tsha256");
 for line in entries.lines(){
  let f:Vec<_>=line.split('\t').collect();assert_eq!(f.len(),3);
  assert!(!f[0].contains("..")&&!f[0].starts_with('/'));
  let b=fs::read(dir.join(f[0].replace('/',"__"))).unwrap();
  let blob=auditbase::blob(&b);let hash=auditbase::h256(&b);
  assert_eq!(b.len().to_string(),f[1]);assert_eq!(blob,f[2]);
  println!("{}\t{}\t{}\t{}",f[0],b.len(),blob,hash);
  assert!(observed.insert(f[0].to_string(),(b.len(),hash)).is_none());
 }
 let manifest=fs::read_to_string(dir.join("MANIFIESTO.tsv")).unwrap();
 let(mut ok,mut mismatch,mut missing)=(0,0,0);
 for line in manifest.lines().skip(1){
  let f:Vec<_>=line.split('\t').collect();assert_eq!(f.len(),4);
  match observed.get(f[0]){
   None=>{missing+=1;println!("AUSENTE\t{}",f[0]);},
   Some((size,hash))=>{
    if size.to_string()==f[1] && hash==f[2]{ok+=1;}
    else{mismatch+=1;println!("DISCREPANCIA\t{}\tdeclarado={} {}\tobservado={} {}",f[0],f[1],f[2],size,hash);}
   }
  }
 }
 println!("IDENTIDADES_ARBOL={} MANIFIESTO_CONCORDANTES={} DISCREPANTES={} AUSENTES={}",observed.len(),ok,mismatch,missing);
 let n=3u64.pow(9)*2;
 println!("RETICULA_PRODUCTO_CARTESIANO={n}; LIMITE_TANDA_DECLARADO=200; seleccion no especificada en protocolo.");
 assert_eq!((observed.len(),ok,mismatch,missing),(17,14,2,3));
 println!("CONTROL_FINALIZADO; ENTREGA_INCOMPLETA; Q1_Q2_NO_EJECUTADOS");
}
