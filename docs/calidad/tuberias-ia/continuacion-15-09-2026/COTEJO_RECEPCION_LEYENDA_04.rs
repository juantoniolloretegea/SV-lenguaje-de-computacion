//! Cotejo documental de las cinco piezas depositadas de LEYENDA-CONTENIDO/4.
//! Rust 1.98.0. Uso: cotejo DIRECTORIO FUENTE_RECEPTOR.
//! Referencias Git: SVperitus-dataset@ea4982398a42eba362f7bc7e9e00f5268f3058c9.
//! SHA-256 del manifiesto: observación receptora; no declaración previa del emisor.
//! Reutiliza funciones históricas de huella; no es validación criptográfica independiente.
#![forbid(unsafe_code)]
...
fn main(){
auditbase::selfcheck();let args:Vec<String>=std::env::args().collect();assert_eq!(args.len(),3);
let cases=[("CONTRATO_CANDIDATO_LEYENDA_CONTENIDO_4.md",12404,"519bb717b5362b900f275d0aad151a7b67bc9b2e","a5026829c381c2e0b47c89e78310887677fd79b8089fffcd78df10bc11e9b03d"),
("SUBSANACION_ENTREGA_04.md",3422,"718799889b7a1aeff6510b73a0fa03ed025f082f","a5543add6e674dcc5e1f129e38d4141769a48b64c38ac98ce8c13fdd0e45bcb1"),
("COTEJO_ENTREGA_04.rs",4274,"879e044649cbad9d0b06dca87967e00c0e41ea3e","7bc2345df02b992bd16b24a522f001d70be166959f3986a1b5392dc6916e72df"),
("COTEJO_ENTREGA_04_SALIDA.txt",1096,"27293f4e778ade4b0583dc1b59c52cd64b2946f5","67e379d09c31bcb99409592c9ad8e7434268e40d6687d51aa524551e853abc40"),
("MANIFIESTO_ENTREGA_04.tsv",1537,"8c2302d032d0d460e091a70b09751d9e7c197f8b","dab91aaf18e8885ce86f0a8ffe12b78c389f52bb12ca0d089bfddb130ba230be")];
for(name,size,blob,sha)in cases{let path=std::path::Path::new(&args[1]).join(name);let b=std::fs::read(path).unwrap();assert_eq!(b.len(),size);assert_eq!(auditbase::blob(&b),blob);assert_eq!(auditbase::h256(&b),sha);println!("{name}\tbytes={size}\tblob={blob}\tsha256={sha}\tCONFORME_IDENTIDAD");}
let b=std::fs::read(&args[2]).unwrap();println!("FUENTE_RECEPTOR\tbytes={}\tblob={}\tsha256={}",b.len(),auditbase::blob(&b),auditbase::h256(&b));
}
