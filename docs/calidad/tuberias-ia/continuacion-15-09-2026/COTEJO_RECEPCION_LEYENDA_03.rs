//! Cotejo documental de los dos adjuntos de LEYENDA-CONTENIDO/3.
//! Rust 1.98.0. Uso: cotejo CONTRATO_3 SUBSANACION_03.
...
let cases=[("CONTRATO_CANDIDATO_LEYENDA_CONTENIDO_3.md",11201,"3da8e1d6e03a9f09ba4ffdbb0e36a5de7b920fa1","9e9937dd11ab74c4ccab2b02f958668daa9677344bce3de5a19ac2d95e858105"),("SUBSANACION_ENTREGA_03.md",2940,"6c6e67d42d6fae857a6fe1789772a682a602d7b3","88ca1bb77524fcf09842082a47db9839dd4417c7dd61a037e0aadfb276ff5e3c")];
for(i,(name,size,blob,sha))in cases.iter().enumerate(){let b=std::fs::read(&args[i+1]).unwrap();assert_eq!(b.len(),*size);assert_eq!(auditbase::blob(&b),*blob);assert_eq!(auditbase::h256(&b),*sha);println!("{name}\tbytes={size}\tblob={blob}\tsha256={sha}\tCONFORME_IDENTIDAD");}
}
