//! Cotejo documental de la adenda de cierre documental de LEYENDA-CONTENIDO/4.
//! Rust 1.98.0. Uso: cotejo DIRECTORIO FUENTE_RECEPTOR.
//! Referencias Git: SVperitus-dataset@23fcae974badc52360493e62346dda21a5976a78.
//! SHA-256 del manifiesto de adenda: observación receptora; no declaración previa.
...
fn main(){
 auditbase::selfcheck();let a:Vec<String>=std::env::args().collect();assert_eq!(a.len(),3);
 let cases=[
 ("ADENDA_CIERRE_DOCUMENTAL_ENTREGA_04.md",4381,"59d01d5b6cd6957c1e4ff93ea31572f356600482","b54525863ba58c7c5c997ba469c9d3b54131cb10cd035c65067c684a11f8aba1"),
 ("MANIFIESTO_ADENDA_ENTREGA_04.tsv",2215,"6622ccfdf7b996c1b8e4dca65b19d92353e0cb82","e7d863935229f3855ba17c2e9b432c3afade80808bf5ed7b97ec08212ca97f0b")];
 for(name,size,blob,sha)in cases{let b=std::fs::read(std::path::Path::new(&a[1]).join(name)).unwrap();assert_eq!(b.len(),size);assert_eq!(auditbase::blob(&b),blob);assert_eq!(auditbase::h256(&b),sha);println!("{name}\tbytes={size}\tblob={blob}\tsha256={sha}\tCONFORME_IDENTIDAD");}
 let b=std::fs::read(&a[2]).unwrap();println!("FUENTE_RECEPTOR\tbytes={}\tblob={}\tsha256={}",b.len(),auditbase::blob(&b),auditbase::h256(&b));
}
