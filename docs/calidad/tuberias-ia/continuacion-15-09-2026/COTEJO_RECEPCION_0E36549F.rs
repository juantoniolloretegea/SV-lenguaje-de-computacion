...
fn main(){
 ...
 let input=std::fs::read_to_string(&a[2]).unwrap();
 println!("ruta\tbytes\tblob_git\tsha256");
 let mut n=0;
 for line in input.lines(){let f:Vec<_>=line.split('\t').collect();assert_eq!(f.len(),3);let b=std::fs::read(root.join(f[0])).unwrap();assert_eq!(b.len().to_string(),f[1]);assert_eq!(auditbase::blob(&b),f[2]);println!("{}\t{}\t{}\t{}",f[0],b.len(),auditbase::blob(&b),auditbase::h256(&b));n+=1;}
 assert_eq!(n,25);println!("25_IDENTIDADES_CONCORDANTES; NO_CUALIFICACION");
}
