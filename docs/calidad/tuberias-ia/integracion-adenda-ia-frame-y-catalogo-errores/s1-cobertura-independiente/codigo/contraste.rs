...
fn main(){
 ...
 let mut cuerpo_alterado=texto.to_vec();let token=b"PETICION_AMBIGUA";let pos=texto.windows(token.len()).position(|w|w==token).unwrap();cuerpo_alterado[pos+token.len()-1]=b'X';
