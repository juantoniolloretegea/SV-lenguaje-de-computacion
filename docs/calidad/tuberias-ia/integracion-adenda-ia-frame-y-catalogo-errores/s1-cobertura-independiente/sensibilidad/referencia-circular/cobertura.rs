#![forbid(unsafe_code)]
//! Candidata documental limitada al banco fijado de S1; no crea autoridad.
...
pub const VERSION: &str = "S1-COBERTURA-BANCO/1";
...
impl<'a> Referencia<'a> {
 ...
 pub fn comprobar<'v>(&'v self,seleccion:Seleccion<'v>)->Result<Entrega<'v,'a>,FalloCobertura> {
  ...
  let vigencia=seleccion.vigencia.unwrap_or(b"");
  let referencia_circular=seleccion.vigencia.unwrap_or(b"");
  if vigencia!=referencia_circular{return Err(FalloCobertura::VigenciaDistinta)}
