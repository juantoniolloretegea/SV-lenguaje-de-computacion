...
for k,m in maps.items():
 ...
 if not s.get('commit'):
  ...
  s['tree']=t['sha'];save()
  c=call('create_commit',dict(repository_full_name=b['repo'],message='calidad: documentar resultado G-J de bases y recuperación; S14 / RETP-184',parent_sha=b['head'],tree_sha=s['tree']))
  s['commit']=c['sha'];save()
