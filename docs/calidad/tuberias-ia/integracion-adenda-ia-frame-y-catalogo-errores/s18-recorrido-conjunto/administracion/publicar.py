...
R=Path(__file__).resolve().parent;fase=sys.argv[1];assert fase in ['apertura','cierre'];CP=R/('PUBLICACION_'+fase.upper()+'.json')
...
for k,m in maps.items():
 ...
 if not s.get('commit'):
  ...
  s['tree']=t['sha'];save()
  c=call('create_commit',dict(repository_full_name=b['repo'],message='calidad: '+fase+' del recorrido conjunto S18 / RETP-'+('189' if fase=='apertura' else '190'),parent_sha=b['head'],tree_sha=s['tree']))
  s['commit']=c['sha'];save()
