#!/usr/bin/env python3
"""Reproduce S15 G/J: orquestación y cotejo; decisiones en Rust/G1."""
...
def run(label,cmd,code=0,env=None,contains=None):
 require(len(logs)<21,'Presupuesto excedido');r=dict(paso=label,argv=list(map(str,cmd)),esperado_exit=code)
 if env:r['entorno_fijado']={k:env[k] for k in ['LG1_FUENTES','LG1_BINARIO','S15_CAPTURAS']}
...
def env(exe,cap):
 cap.mkdir();return os.environ|dict(LG1_FUENTES=sha((D/'FIJACION_PREVIA.json').read_bytes()),LG1_BINARIO=sha(exe.read_bytes()),S15_CAPTURAS=str(cap))
...
try:
 for mode in ['debug','release']:
  ...
  l=d/'liblote.rlib';c=d/'libcobertura.rlib'
 ...
 for mutant,case in [('unificar_u','F05'),('ignorar_io_tardia','F08'),('admitir_negativa','F02')]:
  exe=W/'debug'/mutant;cap=W/'debug'/('capturas-'+mutant)
  run(mutant+' compilación',[a.rustc,'--edition=2021','-C','opt-level=0','-C','overflow-checks=yes','--cfg',mutant,W/'codigo/contraste.rs','--extern',f'g1={W}/debug/libg1.rlib','--extern',f'lote={W}/debug/liblote.rlib','--extern',f'cobertura={W}/debug/libcobertura.rlib','-L',f'dependency={W}/debug','-o',exe]);bins[mutant]=dict(sha256=sha(exe.read_bytes()),bytes=exe.stat().st_size)
  run(mutant+' detección',[exe],code=1,env=env(exe,cap),contains='FALLO '+case)
 require(len(logs)==21,'Presupuesto incompleto');js(W/'RESULTADO.json',dict(version='S15-RESULTADO/1',conforme=True,casos=14,ejecuciones_normales=6,observaciones=84,invocaciones=len(logs),capturas_identicas_por_ejecucion=len(digests),sensibilidades={'unificar_u':'F05','ignorar_io_tardia':'F08','admitir_negativa':'F02'},pared_campana_s=time.monotonic()-start,limite='Receptor sintético y cobertura documental S2. Sin proveedor real, red real, pantalla profesional ni promoción nuclear.'))
except Exception as e:
...
print('S15 conforme dentro de contrato y presupuesto fijados.',flush=True)
