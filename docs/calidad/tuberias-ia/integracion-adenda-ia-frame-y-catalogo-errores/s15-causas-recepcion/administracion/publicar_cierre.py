...
for k,m in maps.items():
 ...
 if not s.get('commit'):
  ...
  s['tree']=t['sha'];save()
  c=call('create_commit',dict(repository_full_name=b['repo'],message='calidad: documentar causas preservadas de recepción; S15 / RETP-186',parent_sha=b['head'],tree_sha=s['tree']))
  s['commit']=c['sha'];save()
