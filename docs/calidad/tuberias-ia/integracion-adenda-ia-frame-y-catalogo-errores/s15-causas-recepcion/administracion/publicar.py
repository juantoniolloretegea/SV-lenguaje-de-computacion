...
for k,m in maps.items():
 ...
 if not s.get('commit'):
  ...
  s['tree']=t['sha'];save()
  c=call('create_commit',dict(repository_full_name=b['repo'],message='calidad: fijar causas de recepción y presupuesto; S15 / RETP-185',parent_sha=b['head'],tree_sha=s['tree']))
  s['commit']=c['sha'];save()
