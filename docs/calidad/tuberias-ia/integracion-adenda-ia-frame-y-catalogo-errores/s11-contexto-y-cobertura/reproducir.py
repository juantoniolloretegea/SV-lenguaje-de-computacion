#!/usr/bin/env python3
"""Reproduce el contraste S11 acotado en Linux, con Rust 1.98.0."""
...
def run(label,cmd,code=0,env=None,contains=None):
 ...
 if env:record['entorno_fijado']={k:env[k] for k in ['LG1_FUENTES','LG1_BINARIO','S11_CAPTURAS']}
 try:
...
assert sha(Path(a.rustc).read_bytes())=='3690cc576ede140504698405d5d8fa3826aaadbe71699c6c4ed0a565d6f493e2'
...
def env(exe,cap):
 cap.mkdir();return os.environ|{'LG1_FUENTES':sha((root/'FIJACION_PREVIA.json').read_bytes()),'LG1_BINARIO':sha(exe.read_bytes()),'S11_CAPTURAS':str(cap)}
def build(d,src):
...
# Mutante 1: elimina la obligación de aportar la vigencia; debe caer en AH03.
