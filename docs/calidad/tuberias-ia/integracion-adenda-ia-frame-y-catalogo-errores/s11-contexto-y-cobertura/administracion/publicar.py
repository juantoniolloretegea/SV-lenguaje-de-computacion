...
for k,m in maps.items():
 ...
 if not s.get('commit'):
  ...
  s['tree']=t['sha'];save()
  c=call('create_commit',dict(repository_full_name=b['repo'],message='calidad: fijar recepción contextual y cobertura S11 antes del contraste; RETP-179',parent_sha=b['head'],tree_sha=s['tree']))
  s['commit']=c['sha'];save()
