...
for k,m in maps.items():
 ...
 if not s.get('commit'):
  ...
  s['tree']=t['sha'];save()
  c=call('create_commit',dict(repository_full_name=b['repo'],message='calidad: cerrar contraste acotado de contexto y cobertura S11; RETP-180',parent_sha=b['head'],tree_sha=s['tree']))
  s['commit']=c['sha'];save()
