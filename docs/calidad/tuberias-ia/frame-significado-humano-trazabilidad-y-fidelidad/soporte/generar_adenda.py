...
NAME='ADENDA_ENCAJE_VISUAL_EXPERTO_AGENTE_Y_LOGO_SV_2026_09_12'
...
baseurl='https://github.com/juantoniolloretegea/SV-lenguaje-de-computacion/blob/main/docs/calidad/tuberias-ia/frame-significado-humano-trazabilidad-y-fidelidad/'
def inline(s):
 s=html.escape(s)
 s=re.sub(r'\[([^\]]+)\]\(([^)]+)\)',lambda m:'<link href="'+html.escape(urljoin(baseurl,m[2]),quote=True)+'" color="#167C80">'+m[1]+'</link>',s)
...
while i<len(lines):
 ...
 if line.startswith('|'):
  ...
  t=Table(data,colWidths=([130,369] if len(rows[0])==2 else [65,217,217]),repeatRows=1,hAlign='LEFT')
 ...
 if line.startswith('!['):
  ...
  maxw,maxh=499,350
  if '10-logo' in p.name:maxw,maxh=370,370
...
doc=SimpleDocTemplate(str(ROOT/(NAME+'.pdf')),pagesize=(595.28,841.89),rightMargin=48,leftMargin=48,topMargin=62,bottomMargin=58,title='Del volcán al encaje visual del SV: experto, agente y representación matemática',author='Juan Antonio Lloret Egea; preparación editorial Watson')
