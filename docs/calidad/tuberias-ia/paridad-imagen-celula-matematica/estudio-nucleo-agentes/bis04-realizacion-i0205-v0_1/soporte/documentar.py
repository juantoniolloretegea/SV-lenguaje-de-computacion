...
import json,hashlib,tarfile,gzip,io,shutil,subprocess
R=Path('manifiesto-sv/checkout');B=R/'docs/calidad/tuberias-ia/paridad-imagen-celula-matematica/estudio-nucleo-agentes';D=B/'bis04-realizacion-i0205-v0_1';W=Path('bis-realizacion/montaje');C=Path('bis-realizacion/campana-01')
def dump(p,x):p.write_text(json.dumps(x,ensure_ascii=False,indent=2)+'\n')
def archive(root,prefix,dest,exclude=()):
 mem=io.BytesIO();manifest={}
 with tarfile.open(fileobj=mem,mode='w') as t:
  for p in sorted(root.rglob('*')):
   if not p.is_file() or p.suffix in exclude:continue
   data=p.read_bytes();rel=prefix+'/'+str(p.relative_to(root));info=tarfile.TarInfo(rel);info.size=len(data);info.mode=0o644;t.addfile(info,io.BytesIO(data));manifest[rel]=hashlib.sha256(data).hexdigest()
 dest.write_bytes(gzip.compress(mem.getvalue(),mtime=0));return manifest
archive(W/'entradas','entradas',D/'ENTRADAS.tar.gz')
evidence=archive(C,'campana-01',D/'EVIDENCIA_NATIVA.tar.gz',('.rlib',));dump(D/'HUELLAS_EVIDENCIA.json',evidence)
res=json.loads((C/'resultados/RESUMEN.json').read_text());resources=json.loads((C/'RECURSOS.json').read_text());assert res['fallos']==0
rows=[]
for i in range(1,27):
 x=json.loads((C/'resultados'/f'I0205-{i:02}.json').read_text());assert x['discrepancias']==[];o=x['observado'];rows.append({'id':x['id'],'resultado':o['resultado'],'guarda':o['primera_guarda'],'guardas_superadas':o['guardas_superadas'],'despachos_observados':o['despachos_observados'],'intentos_despacho':o['intentos_despacho'],'preservacion_comprobada':o['estado_antes']==o['estado_despues'],'recibo':o['recibo'],'captura_sha256':hashlib.sha256(bytes(o['captura']['bytes'])).hexdigest() if o['captura'] else None})
dump(D/'RESULTADOS.json',{'resumen':res,'recursos_del_proceso':resources,'integrados':rows,'sondas':json.loads((C/'ENCAPSULACION.json').read_text()),'corte_ejecutado':json.loads((C/'CORTE.json').read_text())})
for n in ['ejecutar.py','continuar.py','reproducir.py','publicar_previo.py','documentar.py']:shutil.copyfile(Path('bis-realizacion')/n,D/'soporte'/n)
summary='''# Realización nativa C02–C05 · BIS-04 · 0.1

Juan Antonio Lloret Egea y Watson · S22 · RETP-2026-220 · 13 de septiembre de 2026.

**Implementada, compilada y ensayada en Rust.** Los 26 casos integrados coinciden con los oráculos comprometidos: 7 entregas documentales concordantes, 17 rechazos y 2 entregas no acreditadas. Ningún esperado se ha modificado. BIS-04 queda en ejecución acotada; el workflow completo sigue abierto.
...
Se ejecutaron **30 invocaciones de admisión**: 26 casos, 3 repeticiones secuenciales y 1 lectura adicional. Las cuatro sensibilidades modifican evidencia ya producida; no son cuatro ejecuciones nuevas del sujeto. Los 202 casos originales C02–C12, los 24 escenarios originales y las 13 variantes históricas C01 mantienen su contabilidad independiente.

[RESULTADOS.json](RESULTADOS.json) reúne guardas, recibos y conteos. [EVIDENCIA_NATIVA.tar.gz](EVIDENCIA_NATIVA.tar.gz) conserva los registros completos, diagnósticos, órdenes, códigos de salida, estados antes/después y bytes de cada captura. [HUELLAS_EVIDENCIA.json](HUELLAS_EVIDENCIA.json) identifica los contenidos del archivo. Los resultados esperados siguen en sus expedientes previos; sus campos observado permanecen vacíos por conservación histórica.
...
Corte de entrada del Lenguaje: `2067310b30e5aee76120444f0166b01390a04970`; laboratorio: `4705ba49c917cb525eeeba4b39f16686afe1e255`. Fuentes y sondas fijadas antes de ejecutar en laboratorio `ce31385d525c2ed9548c33a10aea208cafc45046` y espejo público `2bce84a27f81b92a78ba71237a83de1f67895444`. PRECOMPROMISO.json es una instantánea histórica de ese corte; README se amplía ahora con los resultados. MANIFIESTO.json identifica el expediente final.

Se conservan tres incidencias de construcción/entorno: faltaba Cargo.lock en el primer intento --locked; se generó offline. El segundo intento detectó una llave ausente en main.rs; se conserva la fuente fallida y el diagnóstico. La construcción siguiente fue correcta. Finalmente faltaba /usr/bin/time; la campaña aún no había arrancado y se sustituyó la medición por os.wait4 del PID del binario. No se alteraron los bancos para corregir estas incidencias. El núcleo emitió 25 advertencias heredadas, conservadas en los logs; el montaje no añadió advertencias de compilación.

En la única ejecución medida del binario completo (conductor, datos confiables, sujeto, observador y escritura de evidencia), el sistema informó **10368 KiB de RSS máximo** y **103721543 ns de duración externa**. El intervalo interno fue 100870607 ns. `size_of::<FixedVector>()` dio **49 bytes** en esta compilación; no incluye el objeto admitido completo ni el proceso. Se efectúa la copia desde el slice IR al array al construir; no se ha medido el número de copias optimizadas ni se acredita ventaja frente a otro almacenamiento. Estos valores son observaciones de laboratorio, no límites constituidos ni una prueba de viabilidad temporal o de memoria.
...
'''
(D/'README.md').write_text(summary)
print('Expediente nativo documentado; resumen sin discrepancias.')
