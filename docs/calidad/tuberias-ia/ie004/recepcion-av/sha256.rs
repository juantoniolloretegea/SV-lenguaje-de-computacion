...
fn es_huella(s:&str)->bool{s.len()==64&&s.bytes().all(|b|b.is_ascii_digit()||(b'a'..=b'f').contains(&b))}
const MAGIC_A:&[u8;8]=b"SVAC0001";
fn marco_a(b:&[u8])->RT<Vec<u8>>{if b.len()>4096{return Err(FalloT::Bytes)}let mut out=Vec::new();out.try_reserve_exact(b.len().checked_add(48).ok_or(FalloT::Entero)?).map_err(|_|FalloT::Reserva)?;out.extend_from_slice(MAGIC_A);out.extend_from_slice(&(b.len()as u64).to_le_bytes());out.extend_from_slice(b);out.extend_from_slice(&sha256(b)?);Ok(out)}
fn validar_marco(b:&[u8])->RT<&[u8]>{
    if b.len()<48{return Err(FalloT::Truncado)}if b.get(..8)!=Some(MAGIC_A.as_slice()){return Err(FalloT::Esquema)}
    let n=u64::from_le_bytes(b[8..16].try_into().map_err(|_|FalloT::Truncado)?);if n>4096{return Err(FalloT::Bytes)}let n=usize::try_from(n).map_err(|_|FalloT::Entero)?;let full=n.checked_add(48).ok_or(FalloT::Entero)?;if b.len()<full{return Err(FalloT::Truncado)}if b.len()!=full{return Err(FalloT::Esquema)}let payload=&b[16..16+n];if b[16+n..]!=sha256(payload)?{return Err(FalloT::Correlacion)}Ok(payload)
}
