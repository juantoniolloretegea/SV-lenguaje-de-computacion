...
try{
  ...
  check('C02',()=>{
    ...
    for(const r of old.resultados)for(const lane of r.transporte){
      ...
      const trace=JSON.parse(t);assert.equal(trace.cuerpo_sha256,hash(b));assert.equal(hash(b),lane.cuerpo_sha256);assert.equal(hash(t),lane.traza_a_sha256);
      ...
      states.push({configuracion:r.id,escenario:lane.scenario,cuerpo_sha256:hash(b),traza_sha256:hash(t),estado_v:lane.v.estado});n++;
    }
