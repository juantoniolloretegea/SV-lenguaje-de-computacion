...
const dir=path.dirname(new URL(import.meta.url).pathname),out=path.join(dir,'resultados-reproduccion-completa'),av=path.join(dir,'../recepcion-av');
const hash=b=>crypto.createHash('sha256').update(b).digest('hex');
if(fs.existsSync(out))throw Error('La campaña ya existe; se conserva sin sobrescribir');
const fix=JSON.parse(fs.readFileSync(path.join(dir,'FIJACION_CORRECCION.json'))),spec=JSON.parse(fs.readFileSync(path.join(dir,'CONTROLES_PUBLICOS.json'))),orig=JSON.parse(fs.readFileSync(path.join(dir,'PROCEDENCIA_Y_ESPERADOS.json')));
...
const montajes=JSON.parse(fs.readFileSync(path.join(dir,'MONTAJES_PUBLICOS.json')));
const results=[];
for(const target of ['nativo','wasi'])for(const profile of ['debug','release']){
 ...
 for(let rep=1;rep<=3;rep++)for(const s of spec.casos){
  ...
  if(s.pipeline&&good){
   ...
   for(let i=0;i<24;i++){const req=extracted.solicitudes[i],wire=Buffer.from(JSON.stringify(req));save(id+'-r'+rep+'-'+req.id+'-solicitud.json',wire);const a=await invoke('r'+rep+'-'+req.id+'-a','receptor-a',montajes.casos.find(m=>m.id===req.id).vigente?['a']:['a','revocado'],wire,4144);let ok=a.record.exit_code===0&&!a.record.error&&!a.record.timeout,error=null,body=null,at=null;
...
const files=fs.readdirSync(out).filter(x=>fs.statSync(path.join(out,x)).isFile()&&!x.endsWith('.bin')&&!x.endsWith('.wasm'));save('MANIFIESTO_RESULTADOS.json',{archivos:files.map(ruta=>{const b=fs.readFileSync(path.join(out,ruta));return{ruta,bytes:b.length,sha256:hash(b)}}),binarios:results.flatMap(r=>Object.entries(r.bins||{}).map(([tipo,b])=>({id:r.id,tipo,...b}))),nota:'Todos los stdout/stderr, metadatos, solicitudes extraídas y cuerpos; binarios reconstruibles desde fuentes fijadas.'});
