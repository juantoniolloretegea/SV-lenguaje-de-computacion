...
function run(dest,input,label,exit=0){
 ...
 const p=spawnSync('/usr/bin/time',['-f','%e %U %S %M','-o',timing,...cmd],{input,timeout:5000,maxBuffer:1048576,env:{PATH:process.env.PATH,LANG:'C.UTF-8'}});
 const ns=Number(process.hrtime.bigint()-start);const raw=fs.existsSync(timing)?fs.readFileSync(timing,'utf8').trim():'';
 medidas.push({id,destino:dest,etiqueta:label,entrada_sha256:hash(input),salida_sha256:hash(p.stdout??Buffer.alloc(0)),duracion_ns:ns,time:raw,status:p.status,signal:p.signal,error:p.error?.message??null});
