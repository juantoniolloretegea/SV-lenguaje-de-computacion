...
r=Path('tmp/diagnosticos-158');o=Path('entregas/diagnosticos-del-compilador-y-procedencia');o.mkdir(parents=True,exist_ok=True)
s=Path('tmp/continuacion-157/candidata');shutil.copytree(s,r/'candidata',dirs_exist_ok=True)
plan='''# RETP-158 · Plan previo de comprobación
...
'''
(o/'PLAN_PREVIO.md').write_text(plan)
inv=[]
for f in ['frontend.rs','grammar_conformance.rs','wellformed.rs','transition_data_wellformed.rs','context_wellformed.rs','lib.rs']:
 p=s/'rust/sv_core/src'/f;b=p.read_bytes()
 inv.append({'path':'rust/sv_core/src/'+f,'sha256':hashlib.sha256(b).hexdigest(),'emisores_lexicos_a_revisar':[{'linea':i,'texto':t.strip()} for i,t in enumerate(b.decode().splitlines(),1) if 'Err(' in t or 'map_err(' in t]})
(o/'INVENTARIO_PREVIO.json').write_text(json.dumps(inv,ensure_ascii=False,indent=2)+'\n')
(r/'COMPROMISO_PLAN.json').write_text(json.dumps({'utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'archivos':{p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in [o/'PLAN_PREVIO.md',o/'INVENTARIO_PREVIO.json']}},indent=2)+'\n')
print('Plan e inventario fijados; candidata copiada')
