use crate::compiler_diagnostics::{ValidationFailure, Cause, DeclarationId};
...
impl<'a> Symbols<'a> {
    fn new(program: &'a IrProgram) -> Result<Self, ValidationFailure> {
...
pub(crate) fn validate_program(program: &IrProgram) -> Result<(), ValidationFailure> {
    ...
    for object in program.objects() {
        if let IrObjectKind::OutputSemantics { mappings } = object.kind() {
            ...
            if !duplicates.is_empty() {
                return Err(ValidationFailure::known(format!(
...
fn validate_object(name: &str, kind: &IrObjectKind, symbols: &Symbols<'_>) -> Result<(), ValidationFailure> {
    match kind {
        ...
        IrObjectKind::CellSpec {
            ...
            if nat_cmp_text(b, "3") == Ordering::Less {
                return Err(format!("CellSpec {name}: b debe ser >= 3").into());
            }
...
fn validate_admissibility_table(
    ...
    let Some(expected_count) = domains.iter().try_fold(1usize, |acc, values| acc.checked_mul(values.len())) else {
        return Err(format!("AdmissibilityTable {name}: cardinalidad no representable").into());
    };
    ...
    for (inputs, output) in table {
        if inputs.len() != domain_sets.len() {
            return Err(format!("AdmissibilityTable {name}: aridad incompatible").into());
        }
        if inputs.iter().enumerate().any(|(i, value)| !domain_sets[i].contains(value.as_str())) {
            return Err(format!("AdmissibilityTable {name}: entrada fuera de codominio").into());
        }
        if !out_values.contains(output.as_str()) {
            return Err(format!("AdmissibilityTable {name}: salida fuera de codominio").into());
        }
        if !seen.insert(inputs.clone()) {
            return Err(format!("AdmissibilityTable {name}: fila duplicada").into());
        }
    }
    if seen.len() != expected_count {
        return Err(format!("AdmissibilityTable {name}: tabla incompleta").into());
    }
    Ok(())
}
...
fn validate_coupled_state(
    ...
    if !nat_eq_usize(n, base_vector.len()) || !nat_eq_usize(n, updated_vector.len()) {
        return Err(format!("CoupledState {name}: longitud incompatible").into());
    }
    ...
    for (index, (before, after)) in base_vector.iter().zip(updated_vector).enumerate() {
        if before != after && !bridge_set.contains((index + 1).to_string().as_str()) {
            return Err(format!("CoupledState {name}: actualización fuera de BridgeSet").into());
        }
    }
    Ok(())
}
...
fn validate_graph(
    ...
    for (source, target, position, connector_name) in edges {
        if !node_set.contains(source.as_str()) || !node_set.contains(target.as_str()) {
            return Err(format!("Graph {name}: arista fuera del conjunto de nodos").into());
        }
        ...
        let (source_cell_name, target_bridges) = match (source_spec, target_spec) {
            ...
            _ => return Err(format!("Graph {name}: nodo no CoupledSpec").into()),
        };
        if !target_bridges.iter().any(|bridge| bridge == position) {
            return Err(format!("Graph {name}: posición fuera de BridgeSet").into());
        }
        ...
        if connector_position != position {
            return Err(format!("Graph {name}: target_position incompatible").into());
        }
        ...
        if connector_codomain != source_codomain {
            return Err(format!("Graph {name}: source_codomain incompatible").into());
        }
        ...
        *indegree.get_mut(target.as_str()).unwrap() += 1;
        if regime == "Simple" && !simple_targets.insert((target.as_str(), position.as_decimal().to_owned())) {
            return Err(format!("Graph {name}: concurrencia en régimen Simple").into());
        }
    }
    ...
    if visited != node_set.len() {
        return Err(format!("Graph {name}: ciclo detectado").into());
    }
    Ok(())
}
...
fn validate_frame_via_c03(
    ...
    for reference in eval_results {
        ...
        let source = match operation {
            ...
            _ => return Err(format!("Frame {name}: eval_results contiene no-Evaluate").into()),
        };
    ...
    for reference in gate_results {
        ...
        let inputs = match operation {
            ...
            _ => return Err(format!("Frame {name}: gate_results contiene no-Gate").into()),
        };
    ...
    for reference in supervision {
        ...
        let (meta_eval, target) = match operation {
            ...
            _ => return Err(format!("Frame {name}: supervision contiene no-Supervise").into()),
        };
...
fn validate_domain(
    ...
    if capture_specs.is_empty() || admissibility_specs.is_empty() || ternarizers.is_empty() {
        return Err(format!("Domain {name}: cadenas de constitución vacías").into());
    }
    ...
    if capture_ids != admissibility_ids {
        return Err(format!("Domain {name}: parameter_id incompatibles").into());
    }
    ...
    if !capture_spaces.is_subset(&ternarizer_spaces) {
        return Err(format!("Domain {name}: observation_space sin Ternarizer").into());
    }
    Ok(())
}
