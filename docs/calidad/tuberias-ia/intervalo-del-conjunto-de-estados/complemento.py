...
save('ESPERADOS_PREVIOS.json',{'utc':utc(),'catalogo_sha256':sha((r/'CATALOGO.json').read_bytes()),'catalogo_rs_sha256':sha((r/'catalogo.rs').read_bytes()),'control_base':{'codigo':101,'contiene':'COUNT_ONE_en: intervalo'},'mutacion_intervalo':{'codigo':101,'contiene':'COUNT_ONE_en: intervalo'},'precedencia_anticipada':{'codigo':101,'contiene':'SEMICOLON_PRECEDENCE_en: causa'},'fuentes_candidatas':{str(p.relative_to(r/'candidata')):sha(p.read_bytes()) for p in sorted((r/'candidata').rglob('*')) if p.is_file()}})
...
row=run('control-base',[exe],101);assert 'COUNT_ONE_en: intervalo' in row['stderr']
shutil.copytree(r/'candidata',work/'mutante')
p=work/'mutante/rust/sv_core/src/frontend.rs';old=p.read_text();needle='self.error_site.set(Some((first, last, false)));';assert old.count(needle)==1
new=old.replace(needle,'self.error_site.set(Some((first, last.saturating_sub(1), false)));');p.write_text(new)
...
row=run('control-intervalo',[exe],101);assert 'COUNT_ONE_en: intervalo' in row['stderr']
shutil.copytree(r/'candidata',work/'mutante-precedencia')
p=work/'mutante-precedencia/rust/sv_core/src/frontend.rs';old=p.read_text()
a=old.index('        let states_last = self.pos;');b=old.index('        self.word("rule")?;',a);part=old[a:b]
assert part.count("        self.sym(';')?;")==1
part=part.replace("        self.sym(';')?;\n",'')+"        self.sym(';')?;\n"
new=old[:a]+part+old[b:];p.write_text(new)
(work/'MUTACION_PRECEDENCIA.patch').write_text(''.join(difflib.unified_diff(old.splitlines(True),new.splitlines(True),fromfile='a/rust/sv_core/src/frontend.rs',tofile='b/rust/sv_core/src/frontend.rs')))
lib=work/'libsv_core_precedencia.rlib';exe=work/'control-precedencia'
run('mutante-precedencia-build',[rust,'--edition=2021','--crate-name','sv_core','--crate-type','rlib',work/'mutante-precedencia/rust/sv_core/src/lib.rs','-o',lib])
run('control-precedencia-build',[rust,'--edition=2021','--cfg','candidate',r/'focal.rs','--extern',f'sv_core={lib}','-o',exe])
row=run('control-precedencia',[exe],101);assert 'SEMICOLON_PRECEDENCE_en: causa' in row['stderr']
save('RESULTADO.json',{'estado':'CONFORME','causas_locales':len(actual),'plantillas_es_en':len(actual)*2,'controles_sensibilidad':3,'base_sin_intervalo_detectada':True,'cierre_excluido_detectado':True,'rechazo_anticipado_detectado':True,'procesos':len(logs),'candidata_modificada':False})
