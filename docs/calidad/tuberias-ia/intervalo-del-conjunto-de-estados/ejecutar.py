...
save('RESULTADO.json',{'estado':'CONFORME_EN_ALCANCE_NATIVO','fuentes_nuevas':48,'fuentes_heredadas_160':94,'esperados_160_sucedidos':2,'fuentes_focales':len(cases),'validas_focales':sum(c['cause']=='OK' for c in cases),'ensamblajes_focales':2*sum(c['cause']!='OK' for c in cases),'observaciones_monofuente':len(observations),'repeticiones':3,'modos':['debug','release'],'corpus':{'validos':14,'invalidos':106,'resultado_heredado_identico':True},'unitarios_por_modo':239,'focales_heredados':29,'esperados_158_sucedidos_desde_159':2,'clientes_negativos':4,'errores_de_acceso':6,'procesos':len(logs),'wasi':False,'navegador':False,'proveedores_externos':0,'cierre_global':False})
