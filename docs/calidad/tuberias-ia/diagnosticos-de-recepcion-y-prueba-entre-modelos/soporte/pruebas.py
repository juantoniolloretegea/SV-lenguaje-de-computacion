...
import json
r=Path('tmp/continuacion-157');o=Path('entregas/diagnosticos-de-recepcion-y-prueba-entre-modelos');s=r/'candidata/rust/sv_core/src'
cases=[('RG01','RG.EMPTY_ACT'),('RG02','RG.EMPTY_ISSUER'),('RG03','RG.EMPTY_VERSION'),('RG04','RG.ACT_TOO_LARGE'),('RG05','RG.ISSUER_TOO_LARGE'),('RG06','RG.VERSION_TOO_LARGE'),('RG07','RG.ISSUER_MISMATCH'),('RG08','RG.VERSION_MISMATCH'),('RG09','RG.DECLARATIONS_MISMATCH'),('RG10','RG.ACT_MISMATCH'),('RG11','RG.ALREADY_ATTEMPTED'),('RG12','RG.GENESIS_REJECTED'),('RG13','RG.EMPTY_ISSUER'),('RG14','RG.ALREADY_ATTEMPTED'),('PI01','RG.EMPTY_ACT'),('PI02','RG.EMPTY_ISSUER'),('PI03','RG.EMPTY_VERSION'),('PI04','RG.ACT_TOO_LARGE'),('PI05','RG.ISSUER_TOO_LARGE'),('PI06','RG.VERSION_TOO_LARGE'),('OC01','OC.FOREIGN_BINDING'),('OC02','OC.FOREIGN_BINDING'),('OC03','OC.CORE_REQUIREMENT'),('OC04','OC.EMPTY_CONTRACT'),('OC05','OC.CONTRACT_TOO_LARGE'),('OC06','OC.EXPECTED_TOO_LARGE'),('OC07','OC.OBSERVED_TOO_LARGE'),('OC08','OC.EXACT_MISMATCH'),('OC09','OC.MISSING_OBSERVATION'),('OC10','OC.CONTRACT_TOO_LARGE'),('E2E01','ACREDITADO'),('E2E02','OC.EXACT_MISMATCH'),('E2E03','OC.EXACT_MISMATCH'),('E2E04','OC.EXACT_MISMATCH'),('E2E05','OC.EXACT_MISMATCH'),('E2E06','OC.EXACT_MISMATCH'),('E2E07','OC.MISSING_OBSERVATION'),('E2E08','ACREDITADO'),('E2E09','ACREDITADO'),('E2E10','ACREDITADO')]
(o/'ESPERADOS.json').write_text(json.dumps({'version':'RECEPTION-REGRESSION/1','autoria':'Watson, fijados antes de ejecutar; banco público no reservado','casos':[{'id':i,'clave':c,'fase':('PrepareReception' if i.startswith('PI') else 'Receive' if i.startswith('RG') else 'PrepareCheck' if i in ['OC01','OC02','OC03','OC04','OC05','OC06','OC10'] else 'Compare')} for i,c in cases]},ensure_ascii=False,indent=2)+'\n')
code=r'''use super::*;
...
'''
p=s/'authority/transitions/reuse_tests/reception_tests.rs';p.write_text(p.read_text()+'\n#[cfg(test)]\nmod diagnostic_tests;\n')
p=s/'authority/transitions/reuse_tests/reception_tests/diagnostic_tests.rs';p.parent.mkdir(parents=True,exist_ok=True);p.write_text(code)
(o/'clientes/no_instalar_diagnostico.rs').write_text('''use sv_core::authority::transitions::reception::PreparedGenesisReception;
use sv_core::requirements::observed::PreparedExactCheck;
fn main() {
 let _ = PreparedGenesisReception::prepare_diagnosed;
 let _ = PreparedExactCheck::prepare_diagnosed;
}
''')
print(len(cases),'observaciones esperadas; 8 pruebas focales')
