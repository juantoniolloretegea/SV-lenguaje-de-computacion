"""Publicación acotada S26 R04, laboratorio y espejo; sin force.
...
files={str(p.relative_to(R)):str(p) for p in (E/'r04').rglob('*') if p.is_file() and '__pycache__' not in p.parts}
...
for p,v in files.items():
    ...
    lab[q]=v
if stage=='previo':pb='10c84db9d330f5aeac1ba8b74f069024c6260e5d';lb='b763f5ab9f0dab59265ab1f2e73573ce954730d5'
else:
    pb=json.loads((C/'previo-publico.json').read_text())['commit'];lb=json.loads((C/'previo-laboratorio.json').read_text())['commit']
b=publicar('juantoniolloretegea/SV-matematica-semantica-cuaternaria','lab/playground-sv-permanente',lab,'docs: S26 sondas custodia lectura R04 '+stage,C/(stage+'-laboratorio.json'),expected_base=lb)
a=publicar('juantoniolloretegea/SV-lenguaje-de-computacion','main',files,'docs: S26 sondas custodia lectura R04 '+stage,C/(stage+'-publico.json'),expected_base=pb)
for p,v in files.items():
