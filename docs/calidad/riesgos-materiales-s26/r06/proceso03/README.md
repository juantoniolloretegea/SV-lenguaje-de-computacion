# S26 R06 · PROCESO03 · Observador superviviente de proceso
...
[PROCESO01](../proceso01/RESULTADOS.md) no pudo abrir el canal Unix: el entorno devolvió EPERM antes de crear el hijo. No se ejecutó ningún caso de comportamiento. PROCESO02 utilizó tuberías anónimas, pero el control positivo detectó un cuerpo JSON mayor que la cuota del decodificador reutilizado; la campaña quedó incompleta. [Sus resultados](../proceso02/RESULTADOS.md) se conservan. PROCESO03 corrige el transporte de la captura sin ampliar esa cuota ni modificar el núcleo.

## Protocolo fijado antes del ensayo

El padre conserva en RAM las aperturas y escribe `apertura.json` antes de lanzar cada hijo. Cada caso reserva dos identificadores instrumentales, `Pnn-A` y `Pnn-B`, ligados al mismo encargo de prueba; inicialmente se selecciona A. El emisor es `observador-R06-PROCESO03`, con ámbito de una instancia de campaña. No hay reutilización entre casos, identidad global, permiso de reintento ni persistencia de la correlación acreditada. B sólo se utiliza para el control explícito de selección en P07.

Dos tuberías anónimas heredadas de `Command/Stdio::piped` separan los mensajes hijo→padre del control padre→hijo. La tubería de mensajes transporta piezas con magia/versionado de ocho bytes `R06P03\0\x01`, longitud de cuerpo `u32` big endian y tres campos JSON: `intento`, `clase`, `contenido`. Las clases admitidas son `fragmento`, `captura`, `informe` y `barrera`. La captura se transporta en fragmentos de hasta 3072 bytes, con desplazamiento exacto y concatenación ordenada de hasta 8192 bytes. La pieza final `captura` contiene contexto y captor; sólo se admite si los bytes acumulados coinciden literalmente con la geometría fijada y los metadatos son concordantes. El conjunto ASCII del caso I0205-01 permite esta división; no se ofrece fragmentación UTF-8 general. Los fragmentos parciales permanecen visibles, pero no equivalen a captura completa. La completitud requiere cabecera, longitud exacta del cuerpo recibido y decodificación válida. La cuota de cuerpo queda alineada con los 8192 bytes del decodificador existente. EOF no constituye un informe. Se conserva el canal literal en `canal.bin` y el prefijo procesado. Esa copia se escribe en la frontera de recepción antes de la espera final, para conservarla aunque la recogida del hijo falle.
...
| P11 | T08 | Cabecera declara 8193 bytes: `cuota_mensaje`; el cuerpo no se admite. Terminación no observada hasta liberar al hijo. |
...
- Cuerpo de mensaje: 8192 bytes. Canal recibido: 65536 bytes más un byte centinela; el exceso se conserva como prefijo y no se analiza. Hasta doce mensajes analizados por caso y dos intentos; no hay cola de reintentos.
...
```text
/opt/sv-rust-1.98.0/bin/cargo build --offline --locked --target-dir /tmp/r06p3-build
/tmp/r06p3-build/debug/sv_s26_r06_proceso03 ../../r05/fixture /tmp/r06p3-debug
/opt/sv-rust-1.98.0/bin/cargo build --release --offline --locked --target-dir /tmp/r06p3-build
/tmp/r06p3-build/release/sv_s26_r06_proceso03 ../../r05/fixture /tmp/r06p3-release
```
...
| Expediente de laboratorio | [PROCESO03](https://github.com/juantoniolloretegea/SV-matematica-semantica-cuaternaria/tree/lab/playground-sv-permanente/laboratorio/tareas-watson/riesgos-materiales-s26/r06/proceso03), con los mismos bytes experimentales. |

El seguimiento de este incremento se identifica por S26/PROCESO03 y la revisión de Sucesos. No se asigna un nuevo identificador RETP. S22/Bis y el orden catálogo/cierre → S24 conservan su estado.
