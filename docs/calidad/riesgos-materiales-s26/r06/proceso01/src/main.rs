...
const MAGIC: &[u8; 8] = b"R06P01\0\x01";
...
fn send(sock: &mut UnixStream, id: &str, class: &str, payload: J) -> R<()> {
    io(sock.write_all(&frame(id, class, payload)?))
}
fn ack(sock: &mut UnixStream) -> R<()> {
    let mut b = [0];
...
fn capture(sock: &mut UnixStream, id: &str, cap: &Capture) -> R<()> {
    let text = std::str::from_utf8(&cap.bytes).map_err(|e| e.to_string())?;
    send(
        sock,
        id,
        "captura",
...
fn child(mode: &str, id: &str, socket: &Path, dir: &Path, fixture: &Path) -> R<i32> {
    let mut sock = io(UnixStream::connect(socket))?;
    io(sock.set_read_timeout(Some(Duration::from_secs(3))))?;
    io(sock.set_write_timeout(Some(Duration::from_secs(3))))?;
    if mode == "plazo" {
        ack(&mut sock)?;
        return Ok(0);
    }
    if mode == "eof_vivo" {
        io(sock.shutdown(Shutdown::Write))?;
        ack(&mut sock)?;
        return Ok(0);
    }
    if mode == "exceso" {
...
impl Ledger {
    ...
    fn receive(&mut self, body: &[u8]) -> R<Option<String>> {
        need(self.events.len() < 8, "cuota_mensajes")?;
        ...
        if let Some(k) = self.ids.iter().position(|x| x == id) {
            ...
            match class {
                "captura" => {
                    ...
                    need(
                        encoded(p)? == encoded(&self.expected_capture)?,
                        "captura_no_concordante",
                    )?;
                    slot.capture = Some(p.clone());
                }
                "informe" => {
                    if k != self.active {
                        slot.late += 1;
                    ...
                    if let Some(first) = &slot.first {
                        ...
                        slot.valid = slot.capture.is_some() && encoded(p)? == self.expected_receipt;
                        if !slot.valid {
...
fn observe(c: &Case, fixture: &Path, dir: &Path, exe: &Path) -> R<bool> {
    ...
    let opening = obj(vec![
        ("emisor", s("observador-R06-PROCESO01")),
        ...
        ("perfil", s("contenido_referentes_documentales_locales")),
    ]);
    ...
    let mut sock = loop {
        match listener.accept() {
            ...
            Err(e) if e.kind() == ErrorKind::WouldBlock => {
                need(
                    io(worker.0.try_wait())?.is_none(),
                    "hijo_termino_antes_del_canal",
                )?;
                need(start.elapsed() < Duration::from_secs(3), "plazo_conexion")?;
                std::thread::sleep(Duration::from_millis(2));
            }
            Err(e) => return Err(e.to_string()),
        }
    };
    io(sock.set_nonblocking(true))?;
    let deadline =
        Instant::now() + Duration::from_millis(if c.mode == "plazo" { 150 } else { 1500 });
    let mut raw = Vec::new();
    ...
    let valid = selected.valid && !selected.conflict;
    let mut checks = vec![
...
fn main() {
    ...
    let r = if a.len() == 7 && a[1] == "hijo" {
        ...
        Err("uso: sv_s26_r06_proceso01 FIXTURE DIRECTORIO_NUEVO".into())
    };
    match r {
