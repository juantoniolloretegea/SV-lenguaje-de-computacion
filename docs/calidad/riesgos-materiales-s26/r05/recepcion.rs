...
impl Observation {
    fn read(path:&Path,phase:&'static str)->Self {
        Self::read_after_open(path,phase,||{})
    }
    fn read_after_open(path:&Path,phase:&'static str,hook:impl FnOnce())->Self {
        use std::io::Read;
        let result=(|| {
            let f=File::open(path).map_err(|e|ReadFailure{stage:"open",cause:format!("{:?}",e.kind()),bytes_seen:0})?;
            hook(); // S26 R05: punto controlado posterior a open y anterior a read.
...
impl MaterialRun {
    ...
    pub fn documentary(&self)->J {
        ...
        obj(vec![("version",s("S26-R05-OBSERVACION/1")),("ruta_custodiada",s(&self.path.to_string_lossy())),
...
fn observed_case(dir:&Path,registry:&TrustedRegistry,expected:&[u8],after_operation:impl FnOnce())->MaterialRun {
    observed_case_with_source_hook(dir,registry,expected,||{},after_operation)
}
fn observed_case_with_source_hook(dir:&Path,registry:&TrustedRegistry,expected:&[u8],after_open:impl FnOnce(),after_operation:impl FnOnce())->MaterialRun {
    let path=dir.join("state.bin");
    let before=Observation::read_after_open(&path,"antes_de_operacion",after_open);
...
#[cfg(test)]
pub(crate) fn run_with_source_injection(dir:&Path,registry:&TrustedRegistry,expected:&[u8],hook:impl FnOnce())->MaterialRun {
    observed_case_with_source_hook(dir,registry,expected,hook,||{})
}
