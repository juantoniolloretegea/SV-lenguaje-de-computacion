"""Prepara copias fijadas y ejecuta siete sondas Rust; Python sólo orquesta.
...
for name in ['probe.rs','conductor.rs','observer.rs','recepcion.rs']:
    shutil.copyfile(E/name,project/name)
...
for i,cmd in enumerate(commands):
    with (out/f'{i:02d}.stdout').open('wb') as stdout,(out/f'{i:02d}.stderr').open('wb') as stderr:
        code=subprocess.run(cmd,stdout=stdout,stderr=stderr,env=env,timeout=120).returncode
