"""Publicación acotada S26 R01, laboratorio y espejo; sin force.
...
files={str(p.relative_to(R)):str(p) for p in E.rglob('*') if p.is_file() and '__pycache__' not in p.parts}
...
for p,v in files.items():
    ...
    lab[q]=v
if stage=='previo':pb='8028193c085a7f1a9e063728569d62f380e1b1de';lb='c017c1c472ed3e1e85b8b1efe8f5df6e6b126879'
else:
    pb=json.loads((C/'previo-publico.json').read_text())['commit'];lb=json.loads((C/'previo-laboratorio.json').read_text())['commit']
b=publicar('juantoniolloretegea/SV-matematica-semantica-cuaternaria','lab/playground-sv-permanente',lab,'docs: S26 inventario material y sondas R01 '+stage,C/(stage+'-laboratorio.json'),expected_base=lb)
a=publicar('juantoniolloretegea/SV-lenguaje-de-computacion','main',files,'docs: S26 inventario material y sondas R01 '+stage,C/(stage+'-publico.json'),expected_base=pb)
for p,v in files.items():
