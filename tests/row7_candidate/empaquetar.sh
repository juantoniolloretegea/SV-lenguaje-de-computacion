...
while IFS= read -r -d '' path; do
  case "$path" in
    LICENSE|rust/sv_core/assets/manifiesto-sv.md|rust/*.toml|rust/*.rs|tests/*.svp|tests/*.json|tests/*.rs|tests/*.sha256|tests/row7_candidate/COMPILACION.md|tests/retorno_cyb/reglas_documentales.mjs|tests/retorno_cyb/consumir.mjs)
