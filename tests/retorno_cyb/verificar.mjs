...
import {operationDefinition,consumeBound,ruleBytes,consumerBytes,documentaryRules} from './enlace_consumidor.mjs';
const {regulated,continuity,regulatedMutants,continuityMutants}=documentaryRules;
...
function expectedRows() {
  const rows=[...pr,...ct].map(c=>({id:c.id,variant:'F0',payload:JSON.stringify(doc(c)),side:null,expected:c.esperado,query:null,consumer:('parametro' in c?'regulated:'+c.parametro:'continuity:'+c.clase)}));
  for(const [version,pairs] of [[3,si],[4,pc]])for(const pair of pairs)for(const state of ['a','b']){
    ...
    for(const variant of ['F0','H','HS'])rows.push({id:`${pair.id}-${state}`,variant,payload:JSON.stringify(variant==='F0'?full:reduced),side:variant==='HS'?JSON.stringify(full):null,
      expected:version===3?JSON.stringify(c[pair.diferencia]):c.esperado,query:version===3?pair.diferencia:null,consumer:version===3?'field:'+pair.diferencia:'continuity:'+c.clase});
  }
...
function expectedContract(row) {
  ...
  const artifacts=specs.map(([id,kind])=>{const bytes=id==='FuenteP'?row.payload:id==='OP'?JSON.stringify(operationDefinition(row.consumer,row.variant)):`testigo declarativo ${id}; versión 1`;return {reference:ref(id,bytes),kind,hex:hex(bytes)};});
  ...
  for(const [id,bytes] of [['ReglasDocumentales',ruleBytes],['ConsumidorDocumental',consumerBytes]])artifacts.push({reference:ref(id,bytes),kind:'OperationDefinition',hex:bytes.toString('hex')});
  return {schema:'LIG/0.1',identifier:'CYB-CONSUMO-0.1',version:'1',
...
export async function verify(report) {
  require(same(Object.keys(report),['schema','rows'])&&report.schema==='CYB-LIG-CONSUMO/0.1','ESQUEMA');
  require(Array.isArray(report.rows)&&report.rows.length===expected.length,'INVENTARIO');
  const outputs=[],insufficient=[];
  for(let i=0;i<expected.length;i++){
    ...
    if(r.variant==='H'){
      let cause;try{await consumeBound(r.contract,operationDefinition(e.consumer,e.variant),r.recovered_hex);}catch(error){cause=error.message;}
      require(cause==='INFORMACION_INSUFICIENTE','H_CONSUMIDA');insufficient.push({id:r.id,cause});
    }else{
      const answer=await consumeBound(r.contract,operationDefinition(e.consumer,e.variant),r.recovered_hex);
  ...
  return {scope:'CONSUMO_DOCUMENTAL_CON_REGLA_LIGADA',operation_definitions_checked:186,insufficient,contract_hashes_recomputed:186,complete_cases:78,full_pair_recoveries:36,side_pair_recoveries:36,reduced_controls:36,pairs,outputs,
...
export async function selftest(report) {
  const attacks=[];
  const attack=async(name,code,change)=>{
    const forged=structuredClone(report);change(forged);let got=null;try{await verify(forged);}catch(e){got=e.message;}require(got===code,`SENSIBILIDAD_${name}_${got}`);attacks.push({name,cause:got});
  };
  await attack('huella_F0','CONTRATO_RECALCULO',r=>r.rows[0].contract_sha256='0'.repeat(64));
  await attack('huellas_H_simetricas','CONTRATO_RECALCULO',r=>{r.rows[79].contract_sha256=r.rows[82].contract_sha256='0'.repeat(64);});
  await attack('fuente_con_huellas_recalculadas','CONTRATO_TESTIGO',r=>{const a=r.rows[0].contract.artifacts.find(a=>a.reference.identifier==='FuenteP');a.hex=hex('{}');a.reference.sha256=hash(Buffer.from('{}'));r.rows[0].contract.instances[0].provenance[0].sha256=a.reference.sha256;r.rows[0].contract_sha256=contractHash(r.rows[0].contract);});
  await attack('identificador_filtrado','CONTRATO_TESTIGO',r=>{r.rows[79].contract.identifier='SI01-a';r.rows[79].contract_sha256=contractHash(r.rows[79].contract);});
  await attack('recuperacion_sustituida','RECUPERACION',r=>r.rows[0].recovered_hex=hex('{}'));
  await attack('fila_duplicada','INVENTARIO',r=>r.rows[1]=r.rows[0]);
  await attack('fila_ausente','INVENTARIO',r=>r.rows.pop());
  await attack('alcance_falso','ESQUEMA',r=>r.row9_closed=true);
  ...
  for(const [name,token,code] of [['duplicada','{"schema":"falso",'+raw.slice(1),'JSON_CLAVE_REPETIDA'],['decimal',raw.replace('"rows":','"extra":7.5,"rows":'),'JSON_NUMERO_FORMA'],['entero_grande',raw.replace('"rows":','"extra":12345678901234567890,"rows":'),'JSON_NUMERO_RANGO'],['sustituto',raw.replace('"rows":','"extra":"\\ud800","rows":'),'JSON_UNICODE']]){
    let got=null;try{await verify(parse(Buffer.from(token)));}catch(e){got=e.message;}require(got===code,`SENSIBILIDAD_${name}`);attacks.push({name,cause:got});
  }
  return attacks;
}
if(process.argv[1]&&resolve(process.argv[1])===fileURLToPath(import.meta.url)) {
  try {
    ...
    if(args.length===1&&args[0]==='--fuentes'){
      ...
      const report=parse(readFileSync(args[0]));const result=await verify(report);
      result.rules_sensitivity=rulesSensitivity();
      if(args[1]==='--autoprueba')result.observer_sensitivity=await selftest(report);
