...
/// Proyección JSON usada exclusivamente para las pruebas diferenciales de R0.
///
/// No constituye el serializador canónico completo del Lenguaje. Se mantiene
/// en `sv_core` para que los adaptadores nativo y WebAssembly ejerzan una sola
/// proyección observable al contrastar el mismo `IrProgram` soberano.
pub fn equivalence_json(program: &IrProgram) -> String {
    let objects = program
        .objects()
        .iter()
        .map(object_json)
...
fn object_json(object: &crate::IrObject) -> String {
    format!(
...
fn operation_json(operation: &crate::IrOperation) -> String {
    format!(
